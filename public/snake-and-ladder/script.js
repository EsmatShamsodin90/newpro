...
function goToSnakeAndLadder() {
    window.location.href = '/snake-and-ladder';  // برای تغییر مسیر به صفحه جدید
}

// نمونه‌ای از استفاده در تشخیص صدا
...
recognition.onresult = function (event) {
    ...
    let numberMatch = command.match(/\b(2|3|4)\b/);
    if (numberMatch) {
        let numPlayers = parseInt(numberMatch[0]);
        startGameWithVoice(numPlayers);
    } else {
        speak("Please say two, three, or four.");
    }
};
